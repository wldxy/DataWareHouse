...
import dataware.util.WordCount;
...
//            BigDecimal runTime = resultSet.getBigDecimal(3);
    ...
    public List<Map> findByActorName(String actorName) throws SQLException {
        Connection connection = DataConnection.getConnection("hive");
        String str = "select movie.title, actor.name from movie join actor_movie on (movie.id=actor.movieid) join actor on (actor.id=actor_movie.actorid) " +
    ...
    public List<Map> findByMovieInfo(Integer year, Integer month, Integer season, Integer day,
        ...
        if (!actor.equals(""))
            sql += " join actor_movie on (movie.id=actor.movieid) join actor on (actor.id=actor_movie.actorid) ";
//        if (!director.equals(""))
//            sql += ", director";
        if (!type.equals(""))
